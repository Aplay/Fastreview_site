...
(function($) {
                    ...
                    $this.find('> legend span:first').prepend('<i class="sabai-icon-caret-down"></i> ').wrap('<a class="sabai-form-collapsible" href="#"></a>').end()
                        .find('a.sabai-form-collapsible:first').click(function () {
                            $(this).toggleClass('sabai-form-collapsed')
                                ...
                                .find('.sabai-form-fields:first').slideToggle('fast');
                            return false;
                        }).end()
    ...
    SABAI.flash = function (message, type, fadeout) {
        ...
        if (typeof(message) == 'string') {
            ...
            var $flash = $('<div class="sabai-' + type + '"><span class="sabai-close"><i class="sabai-icon-remove"></i></span>' + message + '</div>').appendTo($('#sabai-flash'));
            if (fadeout && type !== 'error') {
                SABAI.fadeout($flash);
            } else {
    ...
    SABAI.modal = (function () {
        var _modal,
            ...
            _resizeModal = function (modal, width) {
                var modal_max_height, modal_footer_height;
                if (width === undefined || width === null || width > document.documentElement.clientWidth * 0.8) {
    ...
    SABAI.ajax = function (options) {
            ...
            _handleContent = function (response) {
                ...
                if (o.target === '#sabai-modal') {
                    ...
                    if (o.replace) {
                        ...
                        if (o.scrollTo === null) {
                            if (target.attr('id') == 'sabai-content') {
                                SABAI.scrollTo(target, 1000, -100);
                            } else if (target.attr('id') == 'sabai-inline-content') {
                                SABAI.scrollTo(target, 1000, 0);
                            }
                        } else if (o.scrollTo) {
                ...
                try {
                    ...
                    if (o.onError) { 
                        if (!o.onError(error, $(o.target), o.trigger)) {
                            if (o.onErrorFlash && error.message) {
                                SABAI.flash(error.message, 'error', false);
                            }
        ...
        if (typeof o.data === 'object' && !o.data.hasOwnProperty('__ajax')) {
                ...
                if (o.trigger) {
                    if (o.loadingImage) {
                        if (o.trigger.is('a') && !o.trigger.hasClass('sabai-btn')) {
                            if (o.trigger.has('> i').length) {
                                var original_icon = o.trigger.find('> i'), original_icon_width = original_icon.width();
                                original_icon.addClass('sabai-icon-spinner sabai-icon-spin').width(original_icon_width);
                            } else if (!o.trigger.has('img').length) {
            ...
            complete: function (xhr, textStatus) {
                if (o.trigger) {
                    if (o.loadingImage) {
                        if (o.trigger.is('a') && !o.trigger.hasClass('sabai-btn')) {
                            if (o.trigger.has('> i').length) {
                                o.trigger.find('> i').removeClass('sabai-icon-spinner sabai-icon-spin').css('wdith', 'auto');
                            } else if (!o.trigger.has('img').length) {
                        ...
                        if (xhr.status == 278 || xhr.getResponseHeader('content-type').indexOf('json') > -1) {
                            ...
                            if (o.type == 'get') {
                                ...
                                if (o.pushState && window.history && window.history.pushState){
                                    if (o.target !== '#sabai-modal') {
                                        window.history.pushState(o.target, null, o.url);
                                    }
                                }
                            }
                        }
                ...
                for (dependee in conditions) {
                    ...
                    for (var i = 0; i < events.length; i++) {
                        ...
                        if (type !== 'any') {
                            ...
                            event_data['conditions'][dependee] = condition;
                        }
            ...
            _isConditionMet = function (dependee, type, value) {
                switch (type) {
                    ...
                    case 'values':
                        ...
                        return true;
                    case 'checked':
                        return dependee.prop('checked') === Boolean(value);
                    case 'unchecked':
                        return dependee.prop('checked') !== Boolean(value);
                    case 'empty':
                        return ($.trim(dependee.val()) === '') === Boolean(value);
                    case 'filled':
                        return ($.trim(dependee.val()) === '') !== Boolean(value);
                    case 'collapsed':
                        return (dependee.hasClass('sabai-form-collapsible') && dependee.hasClass('sabai-form-collapsed')) === Boolean(value);
                    case 'expanded':
                        return (dependee.hasClass('sabai-form-collapsible') && dependee.hasClass('sabai-form-collapsed')) !== Boolean(value);
                    default:
    ...
    SABAI.removeOption = function (container, trigger) {
    ...
    // Listen to history popstate event if supported by the browser
    if (window.history && window.history.pushState) {
        $(window).on('popstate', function(event) {
            if (!event.originalEvent.state || !$(event.originalEvent.state).length) {
                //window.location.href = location.href;
            } else {
                SABAI.load(event.originalEvent.state, location.href);
            }
        });
    }
})(jQuery);