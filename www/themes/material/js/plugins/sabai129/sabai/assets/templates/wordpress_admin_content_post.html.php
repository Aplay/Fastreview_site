...
$has_title = isset($form_arr['elements']['content_post_title']);
if ($has_title) {
...
$has_body = isset($form_arr['elements']['content_body']);
if ($has_body) {
...
if (!isset($entity) || !$this->Content_Author($entity)->isAnonymous()) {
...
if (isset($form_arr['elements']['content_post_published'])) {
	$submit_actions[] = array(
        'label_markup' => '<i class="sabai-icon-calendar"></i> ' . __('Published on:', 'sabai'),
    ...
    if (isset($form_arr['elements']['content_post_published'][0]['error'])) {
        $errors[] = sprintf(__('Published on: %s', 'sabai'), Sabai::h($form_arr['elements']['content_post_published'][0]['error']));
    }
...
<?php   if ($has_title):?>
		<div id="titlediv">
			<div id="titlewrap">
				<input type="text" name="content_post_title[0]" size="30" tabindex="1" value="<?php Sabai::_h($title);?>" id="title" autocomplete="off" placeholder="<?php Sabai::_h(__('Enter title here'));?>" />
			</div>
			<div class="inside">
