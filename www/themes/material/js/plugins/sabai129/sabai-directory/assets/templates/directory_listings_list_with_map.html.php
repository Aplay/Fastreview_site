    ...
    SABAI.Directory.googleMap(
        ...
        function (center, bounds, zoom) {
            SABAI.ajax({
                ...
                url: "<?php echo $this->Url($CURRENT_ROUTE, $url_params, '', '&');?>&center=" + center.lat() + "," + center.lng() + "&sw=" + bounds.getSouthWest().lat() + "," + bounds.getSouthWest().lng() + "&ne=" + bounds.getNorthEast().lat() + "," + bounds.getNorthEast().lng() + "&zoom=" + zoom,
                onError: function(error) {SABAI.flash(error.message, "error");}
            });
        },
        <?php if (!empty($center)):?><?php echo json_encode($center);?><?php else:?>null<?php endif;?>,
...
<?php endforeach;?>
    ...
    <div class="sabai-span<?php echo intval($settings['map']['span']);?>">
        <div id="<?php echo substr($CURRENT_CONTAINER, 1);?>-directory-map-container">
            <div class="sabai-directory-map-header"><input id="<?php echo substr($CURRENT_CONTAINER, 1);?>-directory-map-update" type="checkbox" disabled="disabled" /><label for="<?php echo substr($CURRENT_CONTAINER, 1);?>-directory-map-update"><?php echo __('Redo search when map moved', 'sabai-directory');?></label></div>
            <div id="<?php echo substr($CURRENT_CONTAINER, 1);?>-directory-map" class="sabai-googlemaps-map" style="height:<?php echo intval($settings['map']['list_height']);?>px;"></div>
        </div>
    </div>
</div>
