...
class UWfile {
    ...
    public function init() {
        return array(
            'name' => __CLASS__,
            'label' => UsersModule::t('File field'),
            'fieldType' => array('VARCHAR'),
            'params' => $this->params,
            'paramsLabels' => array(
                'path' => UsersModule::t('Upload path'),
            ),
    ...
    public function editAttribute($model, $field, $params = array()) {
        ...
        . (($model->getAttribute($field->varname)) ? '<br/>' . CHtml::activeCheckBox($model, '[uwfdel]' . $field->varname, $params)
            . ' ' . CHtml::activeLabelEx($model, '[uwfdel]' . $field->varname, array('label' => UsersModule::t('Delete file'), 'style' => 'display:inline;')) : '');
    }
