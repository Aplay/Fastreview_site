 ...
 */
class ObjectsImages extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('object, filename, realname', 'required'),
			array('realname', 'filter','filter' =>'trim'),
			array('object, is_main, uploaded_by, verified', 'numerical', 'integerOnly'=>true),
			...
			array('id, object, filename, realname, is_main, uploaded_by, date_uploaded, ip_address, verified', 'safe', 'on'=>'search'),
		);
	}
	...
	public static function getLastImages($limit, $city_id)
	{
		...
		$sql = "SELECT uploaded_by, object, max(date_uploaded) as date FROM objects_images
				LEFT OUTER JOIN objects o
                ON (o.id=objects_images.object) 
                WHERE o.city_id = {$city_id}
			  	GROUP BY uploaded_by, cast(date_trunc('day',date_uploaded) as text), object
			  	ORDER BY date DESC
				LIMIT {$limit} ";
	...
	public function afterDelete()
    {
        ...
        $imagePath = $this->objectid->getFileFolder() . $filename;
        ...
        if(!empty($thumbFolders)){
            foreach($thumbFolders as $size){
                ...
                $thumbPath = $getfolder.'/'.$this->object.'_'.$filename;
	 ...
	 * @return ObjectsImages the static model class
	 */
