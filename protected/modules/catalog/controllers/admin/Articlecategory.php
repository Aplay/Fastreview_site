<?php
class CategoryarticleController extends SAdminController {

	public $layout = '//layouts/admin';
    public $pageTitle = 'Рубрики статей';
    public $active_link = 'categoryarticle';
    public $uploadlogosession = 'categoryarticlelogofiles';

    public $CQtreeGreedView = array(
        'modelClassName' => 'CategoryArticle', //название класса
   ...
   /* public function accessRules() {
    ...
    public function actionCreate() {
        ...
        if (isset($_POST['CategoryArticle'])) {
            $model->attributes = $_POST['CategoryArticle'];
            ...
            if ($model->validate()) {
                ...
                if ($err) {
                    ...
                    $this->redirect(Yii::app()->createAbsoluteUrl('catalog/admin/categoryarticle'));
                }
            } else {
                $this->addFlashMessage($model->errors,'error');
            }
     ...
     public function actionUpdate($id) {
        ...
        if (isset($_POST['CategoryArticle'])) {
            ...
            if ($model->validate()) {
                ...
                if ($parent_id_old == $model->parent_id) { // if sending parent_id == current parrent_id do nothing;
                   ...
                   // $model->refresh();
                    ...
                    if($model->parent_id){
                        ...
                        if(!empty($pos) && isset($childs[$pos-1]) && $childs[$pos-1] instanceof CategoryArticle && $childs[$pos-1]['id'] != $model->id){
                ...
                if ($err === false) {
                    Yii::app()->user->setFlash('success', "Рубрика статей {$model->title} успешно обновлена");
                    $this->redirect(Yii::app()->createAbsoluteUrl('catalog/admin/categoryarticle'));
                } else {
                    Yii::app()->user->setFlash('error', "Ошибка обновления рубрики статей");
                }
    ...
    public function actionReturnMove_Firms()
    {
        ...
        $this->renderPartial('category_move_firms', array(
                ...
                'modelClassName' => 'CategoryArticle'
            ),
            false, true);
    }
    ...
    public function actionDelete($id) {
        ...
        if(!$desc){
            ...
            $error = "Рубрика статей имеет подрубрики:\n ";
            foreach ($desc as $rub) {
                $error .= $rub->title."\n";
            }
        ...
        $count = CategoryArticle::model()->countByAttributes(array('category'=>$model->id));
        if($count > 0){
    ...
    {
        $model = CategoryArticle::model()->findByPk($id);
        if(!$model)
