...
<?php echo nl2br(CHtml::encode($model->text)); ?>
  ...
  <span class="c-gray f-11" style="margin-right:5px;">Полезен ли отзыв?</span> <span class="user_votes"><span onclick="toVoteComment(<?php echo $model->id; ?>, 1);" class="user_pro"><i class="md md-thumb-up"></i></span> <span class="user_num"><?php echo $model->yes?$model->yes:' '; ?></span>  <span class="user_contra" onclick="toVoteComment(<?php echo $model->id; ?>, 0);"><i class="md md-thumb-down"></i></span> <span class="user_contra-num"><?php echo $model->no?$model->no:' '; ?></span></span></div>
	<?php
	}
	?>
	</div>
	...
	if($showcommenttext == true){ 
        ...
        if(!empty($model->user_id)){
         ...
         $criteria->condition = 't.object_pk='.$org_id.' and t.id_parent='.$model->id.'  and t.status='.Comment::STATUS_APPROVED;
         $dataProviderComments = new CActiveDataProvider('Comment', array(
                'criteria' => $criteria,
                'sort'=>array(
         ...
         $this->renderPartial('application.modules.comments.views.comment._review_comments',array('dataProviderComments'=>$dataProviderComments,'model'=>$model,'org_id'=>$org_id, 'countComments'=>$countComments));
