<?php
class SpecController extends SAdminController {

	public $layout = '//layouts/admin';
    public $user;
    public $active_link = 'reviewspec';
    public $pageTitle = 'Отзывы специалистов';
    ...
    public function actionIndex() {
        ...
        $model = new CommentSpec('search');
        $model->unsetAttributes();

        if(!empty($_GET['CommentSpec']))
            $model->attributes = $_GET['CommentSpec'];
        ...
        if(isset($_POST['CommentSpec']))
        {
            ...
            if($model->save()){
                ...
                if(Yii::app()->request->isAjaxRequest){
                    ...
                    $this->redirect(Yii::app()->createAbsoluteUrl('comments/admin/spec'));
                }
    ...
    public function actionUpdatestatus($id, $status)
    {
        ...
        if (!empty($model) && ($status == CommentSpec::STATUS_APPROVED || $status == CommentSpec::STATUS_WAITING))
        {
           $model->status = $status;
