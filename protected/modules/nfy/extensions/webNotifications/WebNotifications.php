...
class WebNotifications extends CWidget {
	...
	public static function initClientScript($method=self::METHOD_POLL) {
        ...
        $cs->registerScriptFile($bu . '/js/jquery.webnotification'.(YII_DEBUG ? '' : '.min').'.js', CClientScript::POS_END);
		if ($method==self::METHOD_PUSH) {
			$cs->registerScriptFile($bu . '/js/sockjs-0.3'.(YII_DEBUG ? '' : '.min').'.js', CClientScript::POS_END);
		}
        $cs->registerScriptFile($bu . '/js/main_new_modified.js', CClientScript::POS_END);
		return $bu;
	}
