<?php
	...
	$assetsManager->registerPackage('jquery');
        $assetsManager->registerPackage('jquery.ui');
	...
	$assetsManager->registerScriptFile(Yii::app()->request->baseUrl.'/js/_gallery_filter/jquery.uniform.min.js', CClientScript::POS_END);
?>
<!doctype html>
<html>
<head>
...
<div id="doc3" class="yui-t6">
		...
		<div class="yui-gc">
			...
			<div class="yui-u" id="topRightMenu">
				<form action="/admin/store/products/" method="get" id="topSearchForm">
					<input type="text" name="Products[name]" id="topSearchBox">
				</form>
			...
			<?php } ?>
				...
				<?php
					if(($messages = Yii::app()->user->getFlash('messages')))
					{
						...
						{
							echo '$.jGrowl("'.CHtml::encode($m).'",{position:"top-right", themeState:""});';
						}
						echo '</script>';
					}
				?>
    ...
    $("#nav li").hover( function () { if ($(this).hasClass("parent")) { $(this).addClass("over"); } }, function () { $(this).removeClass("over"); } );     
