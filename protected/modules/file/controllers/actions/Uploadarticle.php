...
class Uploadarticle extends CAction {

    public function run() {

        $controller = $this->getController();

        Yii::app()->session->remove('articlelogofiles');
         // get the Model Name
       ...
       // $allowedExtensions = array("jpg","jpeg","gif","png");
        ...
        $uploader = new qqFileUploader($allowedExtensions, $sizeLimit, 'articlelogofiles');
        $uploader->inputName = 'tmpLogotip';
        $result = $uploader->handleUpload($folder);

        $datasession = Yii::app()->session->itemAt('articlelogofiles');
