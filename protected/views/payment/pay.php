...
<h2 id="steps_title">Выбор тарифа</h2>    
</div>
<div class="card">
<ul class="nav nav-tabs hide" role="tablist">
   <li role="presentation" class="active"><a id="t3" href="#tab-3" data-steptext="Выбор тарифа" aria-controls="profile" role="tab" data-toggle="tab"></a></li>
...
<div id="tab-1" class="tab-pane animated fadeIn " role="tabpanel">
...
Если у вас несколько филиалов или несколько компаний, то вы сможете их добавить в личном кабинете далее.</div>
    ...
    <div class="col-xs-12">
    	<div class="form-group">
    		<?php  $this->renderPartial('application.views.dashboard._addworktime'); ?>
    	</div>
    </div>
    </div>
...
<div id="tab-3" class="tab-pane animated fadeIn   in active" role="tabpanel"> 
<div style="padding:0 41px">
<div class="text-center m-t-25 p-t-15 block-header-alt">
 ...
 <?php 
$plans = Plan::model()->active()->findAll(array('order'=>'position, id','condition'=>'amount>0'));
if($plans){
  $count = count($plans); 
...
<div class="row pricing-table m-t-10">
  ...
  foreach ($plans as $key=>$plan) {
    $this->renderPartial('application.views.dashboard.plan',array('data'=>$plan, 'key'=>$key, 'count'=>$count));
  }
}
?>
