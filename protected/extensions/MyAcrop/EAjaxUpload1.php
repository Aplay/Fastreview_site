...
class EAjaxUpload extends CWidget
{
	...
	public $css=null;
		...
		$assets = dirname(__FILE__).'/assets';
                ...
                Yii::app()->clientScript->registerScriptFile($baseUrl . '/jQuery-Impromptu/jquery-impromptu.js', CClientScript::POS_HEAD);
                Yii::app()->clientScript->registerScriptFile($baseUrl . '/fineuploader/jquery.fineuploader-3.0.min.js', CClientScript::POS_HEAD);
                Yii::app()->clientScript->registerScriptFile($baseUrl . '/Jcrop/jquery.Jcrop.min.js', CClientScript::POS_HEAD);
                Yii::app()->clientScript->registerScriptFile($baseUrl . '/jquery-uberuploadcropper.js', CClientScript::POS_HEAD);
      
 
                
                $this->css=(!empty($this->css))?$this->css:$baseUrl.'/fileuploader.css';
                Yii::app()->clientScript->registerCssFile($this->css);
                Yii::app()->clientScript->registerCssFile($baseUrl .'/jQuery-Impromptu/jquery-impromptu.css');
                Yii::app()->clientScript->registerCssFile($baseUrl .'/Jcrop/jquery.Jcrop.min.css');
		...
		$config = CJavaScript::encode($config);
               // Yii::app()->getClientScript()->registerScript("FineUploader_".$this->id, "var FineUploader_".$this->id." = new qq.FineUploader($config); ",CClientScript::POS_READY);
               
                Yii::app()->getClientScript()->registerScript("FineUploader_".$this->id, 
   "$('#uploadFile').uberuploadcropper({
					//---------------------------------------------------
					...
					fineuploader: {
						...
						validation: {
                                                  ...
                                                  text: {
                                                uploadButton: 'Загрузить фото &raquo;'
                                              }
					},
					//---------------------------------------------------
					...
					//---------------------------------------------------
					folder          : '/uploads/tmp/', // only used in uber, not passed to server
					cropAction      : '/users/profile/crop', // server side request to crop image
					onComplete      : function(e,imgs,data){ 
						var PhotoPrevs = $('#PhotoPrevs');

						for(var i=0,l=imgs.length; i<l; i++){
							PhotoPrevs.append('<img src=\"/uploads/tmp/'+ imgs[i].filename +'?d='+ (new Date()).getTime() +'\" />');
						}
					}
				});",
