...
class SGridView extends CGridView {
	...
	public function getCustomActions()
	{
		if ($this->hasDeleteAction === true)
		{
			$this->customActions = array(array(
				'label'=>'Удалить',
				'url'=>$this->owner->createUrl($this->deleteAction),
				'linkOptions'=>array(
					'class'=>'actionDelete',
	...
	public function insertDropdownHtml()
	{
		...
		echo strtr('
			...
			<div class="gridViewOptionsMenu">
				...
				'{saveLink}'=>$this->_checkAttributes() ? '<a href="#" data-toggle="modal" data-target="#'.$this->getId().'saveFilterDialog" >'.Yii::t('SGridView.core','Сохранить фильтр').'</a>':'<a class="nonActive">'.Yii::t('SGridView.core','Сохранить фильтр').'</a>',
			));
		echo CHtml::openTag('div', array(
	...
	protected function createSaveDialog()
	{
		return Chtml::script("jQuery('#".$this->getId()."saveFilterDialog').dialog({
			...
			'draggable':false,
			'autoOpen':false,
			'HiddenPropertyValue': '".Yii::app()->request->csrfToken."',
			'buttons':{
