<?php
Yii::import('ext.behaviors.category.DCategoryBehavior');
/**
        ...
        if (isset($items[$parent]) && $items[$parent]){
            foreach ($items[$parent] as $item){
                $resultArray[$item->getPrimaryKey()] = array(
                   ...
                   //  'url'=>$item->getPath(),
                     'url'=>Yii::app()->request->baseUrl . '/' . $item->urlPrefix . $item->{$this->fullurlAttribute},       
    ...
    public function findByPath($path)
    {
        ...
        if (count($domens)==1) {
            ...
            $criteria->mergeWith(array(
                'condition'=>'t.' . $this->aliasAttribute . '=:alias AND t.' . $this->parentAttribute . ' IS NULL',
    ...
    public function getBreadcrumbs($lastLink=false)
    {
        if ($lastLink){
          //  $breadcrumbs = array($this->getOwner()->{$this->titleAttribute} => $this->getOwner()->{$this->urlAttribute});
            $breadcrumbs = array($this->getOwner()->{$this->titleAttribute} => Yii::app()->request->baseUrl . '/' .  $this->getOwner()->urlPrefix . $this->getOwner()->{$this->fullurlAttribute});
        } else {
        ...
        while ($i-- && $this->cached($page)->{$this->parentRelation}){
           // $breadcrumbs[$page->{$this->parentRelation}->{$this->titleAttribute}] = $page->{$this->parentRelation}->{$this->urlAttribute};
            $breadcrumbs[$page->{$this->parentRelation}->{$this->titleAttribute}] = Yii::app()->request->baseUrl . '/' . $page->urlPrefix . $page->{$this->parentRelation}->{$this->fullurlAttribute};
