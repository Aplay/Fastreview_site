...
class LinkListPagerAjax extends CBasePager
{
	/**
	...
	public function run()
	{
		...
		//echo CHtml::dropDownList($this->getId(),$selection,$pages,$this->htmlOptions);
               ...
               echo '<input id="gotoajax"  class="pager_input" type="text" value="'.$this->generatePageText($this->getCurrentPage()).'" url="'.$selection.'"  />';

                echo '<span class="lis_sum"> из '.$pageCount.'</span>';
	}
	...
	{
		if($url === null)
			$url = CHtml::asset(Yii::getPathOfAlias('ext.widgets.MyListView.pager1').'.css');
