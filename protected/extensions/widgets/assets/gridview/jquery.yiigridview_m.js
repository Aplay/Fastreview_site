...
(function ($) {
	...
	selectCheckedRows = function (gridId) {
		...
		table.children('.divtbody').find('input.select-on-check').filter(':checked').each(function () {
			$(this).closest('.divtr').addClass('selected');
		});

		table.children('.divthead').find('.divth input').filter('[type="checkbox"]').each(function () {
	...
	methods = {
		...
		init: function (options) {
			...
			return this.each(function () {
				var eventType,
					...
					sortSelector = '#' + id + ' .' + settings.tableClass + ' .divthead .divth a.sort-link',
				...
				if (settings.selectableRows > 0) {
					...
					$(document).on('click.yiiGridView', '#' + id + ' .' + settings.tableClass + ' > .divtbody > .divtr', function (e) {
						var $currentGrid, $row, isRowSelected, $checks,
							$target = $(e.target);

						if ($target.closest('.divtd').is('.empty,.button-column') || (e.target.type === 'checkbox' && !$target.hasClass('select-on-check'))) {
							return;
						}
					...
					if (settings.selectableRows > 1) {
						$(document).on('click.yiiGridView', '#' + id + ' .select-on-check-all', function () {
							var $currentGrid = $('#' + id),
								...
								$rows = $currentGrid.find('.' + settings.tableClass).children('.divtbody').children();
							if (this.checked) {
		...
		getRow: function (row) {
			...
			return this.find('.' + sClass).children('.divtbody').children('.divtr').eq(row).children();
		},
		...
		getColumn: function (column) {
			var sClass = gridSettings[this.attr('id')].tableClass;
			return this.find('.' + sClass).children('.divtbody').children('.divtr').children('.divtd:nth-child(' + (column + 1) + ')');
		},
		...
		getSelection: function () {
			var settings = gridSettings[this.attr('id')],
				...
				selection = [];
			this.find('.' + settings.tableClass).children('.divtbody').children().each(function (i) {
		...
		getChecked: function (column_id) {
			...
			this.find('.' + settings.tableClass).children('.divtbody').children('.divtr').children('.divtd').children('input[name="' + column_id + '"]').each(function (i) {
				if (this.checked) {
