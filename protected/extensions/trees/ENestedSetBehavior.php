...
class ENestedSetBehavior extends CActiveRecordBehavior
{
	...
	public function getParentWithParams($lft,$rgt,$root='')
	{
		...
		$criteria->mergeWith(array(
			'condition'=>$alias.'.'.$db->quoteColumnName($this->leftAttribute).'<'.$lft.
				' AND '.$alias.'.'.$db->quoteColumnName($this->rightAttribute).'>'.$rgt,
		...
		{
			$criteria->addCondition($alias.'.'.$db->quoteColumnName($this->rootAttribute).'='.CDbCriteria::PARAM_PREFIX.CDbCriteria::$paramCount);
			$criteria->params[CDbCriteria::PARAM_PREFIX.CDbCriteria::$paramCount++]=$root;
		}
