...
class FileARBehavior extends CActiveRecordBehavior
{
    ...
    /**
     * without the first and last /, the folder where to put the image relative to webroot.model. must be set.
     */
    public $relativeWebRootFolderModel;
    ...
    protected function getFileName()
    {
        if (!isset($this->_fileName)) {
            if (!isset($this->attributeForName)) {
                $this->attributeForName = $this->owner->tableSchema->primaryKey;
 
            }
            if (!is_array($this->attributeForName)) {
                $partName = $this->owner->{$this->attributeForName};
            } else {
                $partName = array();
                foreach ($this->attributeForName as $attr) {
                    $partName[] = $this->owner->{$attr};
                }
    ...
    protected function process($processor, $options)
    {
        foreach ($options as $method => $args) {
            if($method == 'adaptiveResizeQuadrant'){
                if($this->imsize[1]/$this->imsize[0] > 1.6){
                    $args[] = 'T';
                } else {
                    $args[] = 'C';
                }
    ...
    public function afterSave($evt)
    {
        ...
        if(!empty($this->extension) && isset($file))
            $valid = strpos(strtolower($this->extension), strtolower($file->extensionName)) !== false;
        else
            $valid = true;
        if ($file && $valid) {
            $path = $this->getFolderPath();
            ...
            if(!$this->saveFile($file, $path, $fname, $file->extensionName)){
                $this->owner->saveAttributes(array($this->attribute=>''));
            } else {
                $this->owner->saveAttributes(array($this->attribute=>$fname.'.'.$file->extensionName));
            }
        }
    }
    ...
    protected function saveFile($file, $filePath, $fileName, $ext)
    {
        ...
        $this->imsize = @getimagesize($file->tempName); // 0-ширина, 1-высота
        if(!$this->imsize || !isset($this->imsize[0]) || !isset($this->imsize[1]) || empty($this->imsize[0]) || empty($this->imsize[1]))
            return false;
        ...
        if (count($this->formats) == 1 && !isset($klass)) {
            ...
            foreach ($this->formats as $f) {
                ...
                if(!$processor->save($path . $f['suffix'] . $real_ext)){
                    $ret = false;
                }
            }
        } else {
    ...
    private function makeFileUrl($fileName)
    {
        ...
        if($this->relativeWebRootFolderModel && $this->owner->id){
            $path = $path = Yii::app()->baseUrl . '/' . $this->relativeWebRootFolder . '/' . $this->relativeWebRootFolderModel . '/' . $this->owner->id . '/'. basename($fileName) . $suffix;
        } else {
            $path = Yii::app()->baseUrl . '/' . $this->relativeWebRootFolder . '/' . basename($fileName) . $suffix;
